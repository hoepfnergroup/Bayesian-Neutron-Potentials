def mie_fluid(path, it, N_particles, spacing, name, timestep, n, sigma, epsilon, kbT, density, eq_time, production_time):
    import math
    import numpy as np
    import itertools
    import hoomd
    ...
    with gsd.hoomd.open(name= path+'/lattice/lattice' + str(it) +'.gsd', mode='w') as f:
        f.append(snapshot)

    # Pair hoomd to the cpu and create basic sim objects
    cpu = hoomd.device.CPU()
    sim = hoomd.Simulation(device=cpu, seed=1)
    sim.create_state_from_gsd(filename=path+'/lattice/lattice' + str(it) +'.gsd')
    ...
    gsd_writer = hoomd.write.GSD(filename= path + '/traj/mie' + str(it) +'.gsd',
